...
def aktualisiere_repertoire_tabelle(bestehende_daten, neue_zeilen):
    ...
    betroffene_schueler = {
        str(zeile["SchÃ¼ler"]).casefold() for zeile in neue_zeilen
    }
    behalten_maske = ~bestehende_daten["SchÃ¼ler"].fillna("").astype(str).str.casefold().isin(
        betroffene_schueler
    )
...
def speichere_unterrichtspaket(archiv_zeile, repertoire_zeilen):
    ...
    if repertoire_zeilen:
        betroffene_schueler = {
            str(zeile["SchÃ¼ler"]).casefold() for zeile in repertoire_zeilen
        }
        repertoire_speicher[:] = [
            zeile for zeile in repertoire_speicher
            if str(zeile.get("SchÃ¼ler", "")).casefold() not in betroffene_schueler
        ]
...
#_________________________________ALTE VERSION ________________________________
